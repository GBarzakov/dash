...
const unsigned long BT_INTERVAL = 50;   // до ~20 пакета/сек

struct __attribute__((packed)) BtData {
    uint32_t id = 0;
    ...
    uint16_t map = 0;
    int16_t  mat = 0;        // IAT
    int16_t  clt = 0;        // Coolant
    int      tps = 0;

    uint16_t batt_raw = 0;
    uint16_t batt = 0;

    float    afr = 0.0f;     // ако при теб е скалирано цяло, ще го кастваме
...
float afr_g   = 0.0f;   // да е float, всички ECU-та го смятат като float
float fuel_p  = 0.0f;   // единствена променлива за fuel pressure
float oil_p   = 0.0f;   // единствена променлива за oil pressure
int   oil_temp = 0;     // маслена температура
...
btData.id        = rx_message.identifier;

btData.rpm       = rpm_g;
btData.map_raw   = map_gRaw;
btData.map       = map_g;
btData.mat       = mat_g;
btData.clt       = clt_g;
btData.tps       = tps_g;

btData.batt_raw  = batteryVoltageRaw;
btData.batt      = batteryVoltage;

// важната част:
btData.afr           = afr_g;   // вече е float
btData.fuel_pressure = fuel_p;  // от CAN кода
btData.oilPressure   = oil_p;   // от CAN кода
btData.oil_temp      = oil_temp;

btData.ignang    = ignang;
btData.injpw     = injpw;
btData.egt       = egt;
...
void sendBluetoothTelemetry() {
    ...
    if (!btData.hasData) return;

    unsigned long now = millis();
    if (now - lastBtSend < BT_INTERVAL) return;
    lastBtSend = now;
    ...
    int n = snprintf(
        line, sizeof(line),
        "ID=%X;"
        "RPM=%u;MAPraw=%u;MAP=%u;MAT=%d;CLT=%d;TPS=%d;"
        "BATTraw=%u;BATT=%u;"
        "AFR=%.2f;"
        "FUEL_PRESS=%.2f;OIL_PRESS=%.2f;"
        "OIL_P=%.2f;FUEL_P=%.2f;"
        "OIL_TEMP=%d;"
        "IGN=%.2f;INJPW=%.3f;EGT=%u\n",
        btData.id,
        btData.rpm, btData.map_raw, btData.map, btData.mat, btData.clt, btData.tps,
        btData.batt_raw, btData.batt,
        btData.afr,
        btData.fuel_pressure, btData.oilPressure,
        btData.oilPressure, btData.fuel_pressure,
        btData.oil_temp,
        btData.ignang, btData.injpw, btData.egt
    );

    if (n > 0) {
        if (n >= (int)sizeof(line)) {
            SerialBT.write((const uint8_t*)line, sizeof(line) - 1);
        } else {
            SerialBT.write((const uint8_t*)line, n);
        }
...
float last_flex_content = -1.0f;  // Последна известна стойност за Flex Content
 ...
 void setup() {
    ...
    Serial.println("Booting miniblut2...");
    ...
    // Bluetooth - абсолютно директно, нищо условно
    Serial.println("[BT] init...");
    bt_ok = SerialBT.begin("BGauge");
    if (bt_ok) {
        Serial.println("[BT] OK, device name: BGauge");
    } else {
